#!/usr/bin/env bash
# Static checks for the agent-rules skill.
...
echo "Checking agent-rules skill at: $SKILL_DIR"

# 1. skill.json conforms to canonical schema and matches the directory
validate_skill_json_contract "$REPO_ROOT" skill.json "agent-rules"
...
if grep -q "Create tracking state" SKILL.md &&
   grep -q "agent-rules-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md" SKILL.md &&
   grep -q "agent-rules-workflow-state-<YYYY-MM-DD>-<scope-slug>.json" SKILL.md; then
...
if grep -q "audit-artifacts/agent-rules-" SKILL.md; then
  ok "SKILL.md preserves tracking fallback path"
else
  err "SKILL.md missing audit-artifacts/agent-rules fallback path"
fi
...
validate_trigger_evals_contract "$REPO_ROOT" evals/trigger-evals.json "agent-rules"
