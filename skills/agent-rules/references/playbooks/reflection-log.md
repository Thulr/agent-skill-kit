...
- **Rules expire; promotion is not forever.** A rule promoted to close a
  model limitation becomes dead weight once the model no longer needs
  it. Scaffold deletion is a first-class obligation, not an afterthought
  — the same log that promotes rules must also flag them for retirement,
  or the `AGENTS.md` rots into stale, contradictory prose (harness
  accretion).
...
- **H5.** If the recurring gap is *architectural boundary violations*
  ...
  model; use `agent-readiness` gates guidance to enforce it.
- **H6. Prune/expire promoted rules.** When promoting, record an expiry
  ...
  On each `promote` pass, re-check already-promoted rules: if the
  underlying limitation no longer reproduces (no new same-gap entries,
  the current model handles it), propose *removing* the rule / hook /
...
- **H1.** Assume Levels 1–3 already scored by
  `agent-readiness`. Require user to state the current
...
- "AGENTS.md" — Agentic AI Foundation; the instruction-surface
  ...
  `agent-readiness`, not this skill).
