...
Two axes, not one ladder. Beyond *how much* isolation, pick the *isolation style*: an **OS-level
profile** (Seatbelt / bubblewrap wrapping the agent process — low overhead, tight containment of a
process already on the host; this is what Claude Code's sandbox is) versus a **microVM / container**
(agent runs disposable and cross-platform, looser autonomy). Run containers under a **rootless engine**
(rootless Podman / rootless Docker) so a prompt-injection compromise has no host root — "use Docker" is
weaker advice than "use rootless containers." The same image should lift unchanged from local run to CI
...
- **Credential exposure.** Without container isolation, model-generated code runs with harness
  credentials. OpenAI's position: separate harness from compute. Isolate at the strongest available
  wall: keep the secret out of the agent *process* (a broker holds it), out of its *network reach* (a
  host-side egress proxy injects auth by host/path so the agent never sees the key even with a malformed
  URL), and prefer *token exchange* — an RFC 8693-style broker exchange that mints tokens scoped and
  short-lived *by policy* — over a long-lived secret. (W10)
...
- **H4.** Verify network egress: default-deny outbound with a per-domain allow-list; unrestricted
  egress equals no sandbox for exfiltration. An `HTTP_PROXY`-only block is *cooperative* — a binary that
