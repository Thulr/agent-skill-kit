...
<2–4 sentences: what agent-facing surface was reviewed, what the headline score means, the
single most load-bearing finding (a trust-boundary gap or an unrecoverable error path
outranks tidiness).>
...
1. **<AGENT-DX-...>** — <why this one>. _Verify by:_ <narrow check>.
2. **<AGENT-DX-...>** — <why>. _Verify by:_ <narrow check>.
3. **<AGENT-DX-...>** — <why>. _Verify by:_ <narrow check>.

## Findings

Ordered by severity (4 → 0). Each finding cites the playbook heuristic that surfaced it and
carries a stable ID with the surface prefix: `AGENT-DX-SDK`, `AGENT-DX-TOOL`, `AGENT-DX-OUT`,
`AGENT-DX-ERR`, or `AGENT-DX-TEL`.

### AGENT-DX-SDK-001 — <short name>

- **Severity:** <0–4>
- **Status:** discovered
- **Heuristic:** <playbook heuristic name and intent tag>
- **Evidence:** <file:line, snippet, or pattern>
- **Why this matters:** <one sentence — what a consuming agent does wrong because of it>
...
`docs/audits/agent-dx-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md` and the workflow state from
`templates/workflow-state.json` at
`docs/audits/agent-dx-workflow-state-<YYYY-MM-DD>-<scope-slug>.json`. Create the directory if
needed. If the target is not a repo or `docs/audits/` is not writable, use matching
`audit-artifacts/agent-dx-...` paths instead. Report both saved paths; do not merely
