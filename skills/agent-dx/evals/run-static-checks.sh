...
python3 "$repo_root/scripts/check-skill-static.py" \
  ...
  --require-pattern 'audit report preserves fallback path::templates/audit-report.md::audit-artifacts/agent-dx-' \
  --require-pattern 'report has Later/as-it-grows bucket::templates/audit-report.md::as it grows' \
  --require-pattern 'sdk-design playbook H1::references/playbooks/sdk-design.md::^# SDK Design Playbook' \
  --require-pattern 'tools-and-mcp trust boundary::references/playbooks/tools-and-mcp.md::lethal trifecta' \
  --require-pattern 'errors playbook retry shape::references/playbooks/errors-and-retry.md::Shape tool errors for retry'
