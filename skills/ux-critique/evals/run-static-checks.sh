...
# ----- Tracking behavior gates (ux-critique ships findings-ledger + workflow-state) -----
...
if [[ -f "$router" ]]; then
  ...
  [[ -d "$skill_dir/references/intents" ]] && fail "references/intents/ found: ux-critique is a one-layer router (no surface layer)"
