...
# ----- File presence (design family is lean: no tracking artifacts) -----
check_file "$skill_md"
...
check_pattern 'optimize-plan is profile-first' '^## Profile' "$skill_dir/templates/optimize-plan.md"
check_pattern 'optimize-plan has verification gate' '^## Verification' "$skill_dir/templates/optimize-plan.md"
check_pattern 'strategy-doc has adoption sequence' '^## Adoption sequence' "$skill_dir/templates/strategy-doc.md"

# ----- Intent router structure (design family = design, optimize, strategize) -----
if [[ -f "$intent_router" ]]; then
  rows=$(grep -cE '^(design|optimize|strategize),' "$intent_router")
...
for surface in $all_surfaces; do
  ...
  awk '/^## Heuristics/{f=1;next} /^## /{f=0} f && /\((audit|design|diagnose|optimize|strategize)/{found=1} END{ if(!found) exit 1 }' \
