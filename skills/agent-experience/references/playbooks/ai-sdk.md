...
- **MCP as a tool source** *(design, audit)* — if the SDK accepts
  tools, MCP servers are one of the accepted shapes; in-process MCP
  is supported so custom tools do not require a subprocess.
