...
3. **Load grounded context.** Load only the playbook(s) for the chosen sub-surfaces, plus `references/empirical-warnings.md` and the appropriate rubric from `references/intent-router.csv` row (`core/maturity-rubric.md` for `assess`; `core/severity-rubric.md` otherwise).
4. **For `scaffold`: collect observed failures.** Ask the user for 3–5 concrete agent failures observed on this repo before generating any file. Refuse to scaffold without justification (empirical warning W1).
