...
- **H5.** Check `AGENTS.md` and any instruction file for injection-bait patterns — `<!-- ignore
  ...
  as code under review, not documentation. (severity cap: 5; lens: adversarial)
...
- **Do not autogenerate from boilerplate or `/init`.** Mündler et al. (arXiv:2602.11988) found
  ...
  must cite at least one observed agent failure — no placeholder rules, no generic "write clean
  code" copy.
