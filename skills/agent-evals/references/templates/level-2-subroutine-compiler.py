...
METRIC GAMING WARNING: an optimizer maximizes whatever the metric rewards. If
the metric has cracks (a leaky exact-match, a judge with a loophole, a check a
degenerate output can satisfy), the optimizer will learn to exploit them rather
than do the task -- the reward-hacking failure mode. Before trusting a compiled
module, inspect a sample of its rollouts/traces, and confirm the metric cannot
be satisfied by a degenerate solution (cf. an AST / anti-hacking gate). A rising
score with unchanged real quality is the tell.
"""
import dspy
...
# Exact-match is strict here, but for fuzzier tasks a weak/leaky metric is the
# attack surface the optimizer will exploit (see METRIC GAMING WARNING above).
# Pair any judge-based metric with rollout inspection before trusting the score.
