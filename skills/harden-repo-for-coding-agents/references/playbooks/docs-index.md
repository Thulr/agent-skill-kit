...
Use `docs-audit` / `docs-design` instead when the target is a public docs/help
...
- **llms.txt reduces hallucinated endpoints — but it's delivery, not correctness.** A flat index of
  curated, plain-text documentation links lets agents reach current docs instead of leaning on stale
  pre-training weights, so they hallucinate fewer API parameters; `llms-full.txt` concatenates the
  critical docs into one ingestible file. It improves *delivery*: the agent still has to choose what to
  load and when to stop, so llms.txt is not a substitute for context engineering and does not by itself
  verify correctness.
- **Vendored read-only reference trees are a deliberate context surface.** A `repos/` or `vendor/`
  tree of pinned upstream source the agent may *read but not edit* — declared "do not edit" in
  AGENTS.md and excluded from the write path / editor watch — gives ground-truth reference without
  inviting the agent to mutate dependencies. A legibility move, not just a build artifact.
