...
check_pattern 'frontmatter name' '^name:[[:space:]]*informed-skill-curator$' "$skill_md"
...
if (( failures > 0 )); then
  printf '\ninformed-skill-curator static eval failed with %d issue(s).\n' "$failures" >&2
  exit 1
fi

printf 'informed-skill-curator static eval passed.\n'
