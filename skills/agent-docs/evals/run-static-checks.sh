...
python3 "$repo_root/scripts/check-skill-static.py" \
  ...
  --require-pattern 'audit report preserves fallback path::templates/audit-report.md::audit-artifacts/agent-docs-' \
  --require-pattern 'report has Later/as-it-grows bucket::templates/audit-report.md::as it grows' \
  --require-pattern 'agents-md playbook H1::references/playbooks/agents-md.md::^# AGENTS.md Contract Playbook'
