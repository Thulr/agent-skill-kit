...
if [[ -f "$skill_md" ]]; then
  ...
  grep -Eq '^name: agent-evals$' "$skill_md" || fail "SKILL.md frontmatter must include: name: agent-evals"
...
# Mirrors the per-domain audit/design skills: no author last name or source title in SKILL.md.
...
validate_skill_json_contract "$repo_root" "$skill_json" "agent-evals"
validate_trigger_evals_contract "$repo_root" "$trigger_evals" "agent-evals"

if (( failures > 0 )); then
  printf '\nagent-evals static eval failed with %d issue(s).\n' "$failures" >&2
  exit 1
fi

echo "agent-evals static eval passed."
