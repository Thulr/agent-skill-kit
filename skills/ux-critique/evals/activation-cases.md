# ux-critique Eval Cases

Activation + behavioral cases for `ux-critique` — critiquing an existing
...
`ui-design`; developer-facing surfaces are `dx-critique`. Cases that route to
those siblings appear here as **negatives**.

## Static verification

```bash
bash skills/ux-critique/evals/run-static-checks.sh
```
...
(`ux-critique-` ledger/workflow-state prefixes).
...
`docs/audits/ux-critique-findings-ledger-…md` and `…workflow-state-…json` (or
the `audit-artifacts/ux-critique-…` fallback); reports both paths.
**Fail if:** only offers to track, or emits the ledger inline without saving.

## Case 5 — Closeout from saved state
**Prompt:** `Verify whether UX-FORM-003 is fixed in this PR using docs/audits/ux-critique-workflow-state-2026-05-22-signup.json.`
...
**Expected:** recognizes a developer-facing surface; defers to `dx-critique`.

## N4 — Developer-facing API audit
**Prompt:** `Audit our public REST API for friction — error envelope, pagination, idempotency.`
**Expected:** defers to `dx-critique` despite the "audit" keyword.
