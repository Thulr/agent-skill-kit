...
- **Aggregate scores hide where it broke.** A single pass-rate can't separate a guardrail regression
  (block ship) from a capability/north-star dip (expected while a hill is being climbed), and a
  final-output grade misses trajectory failures (loops, wrong tool, lost handoff). Tag cases by role
  and grade the path, not only the answer.
...
- **H6.** Verify eval cases carry a role: *capability* (a hill being climbed — a red is intentional)
  vs *regression* (a ship-blocker), and *guardrail* (block on red) vs *north-star* (report on red).
  Without the tags, CI can't tell an intentional capability miss from a real regression, and a red
  build means nothing. (severity cap: 3; lens: maintainer)
- **H7.** For agent (multi-step) evals, confirm grading is trajectory-aware — tool sequence, no-loop
  integrity, step budget, handoff context — not only the final output. A run where every step
  "passed" can still fail the task. (severity cap: 3; lens: cold-agent)
...
- **H6.** CI blocks on the wrong reds → tag each case `capability | regression` and
  `guardrail | north-star`; gate the build only on regression/guardrail reds and report
  capability/north-star deltas without failing. Add trajectory/process assertions (tool sequence,
  no-loop, step budget) where final-output grading misses path failures.
...
- "SWE-bench" — executable repo-level benchmark (fail-to-pass / pass-to-pass; Docker harness; the
  model is not shown the grading tests); the canonical reference for task-level eval design over
  isolated snippet tests. (SWE-agent is the coding *agent* in that ecosystem, not the benchmark.)
  For side-effectful CLI/OS tasks, Terminal-Bench is the companion execution benchmark.
- "Trajectory evaluation" — grade the *path*, not just the final answer: step efficiency, tool-selection
  relevance, parameter validity, reasoning coherence, no-loop integrity. A run that reaches the right
  answer via 5× the tool calls scores 1.0 on output grading but fails here.
