...
- **Bare invocation** (`"set up reflection log"`, `"use agent-rules"`): show the intent
...
5. **assess-l4l5.** Assume Levels 1–3 are already scored by `agent-readiness` (confirm
...
7. **Create tracking state.** For `assess-l4l5` with 7+ gaps, a level-ceiling blocker, or
   ...
   `docs/audits/agent-rules-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md` and workflow
   state at `docs/audits/agent-rules-workflow-state-<YYYY-MM-DD>-<scope-slug>.json` (fall
   back to `audit-artifacts/agent-rules-{findings-ledger|workflow-state}-<YYYY-MM-DD>-<scope-slug>.{md|json}`
...
- `references/core/maturity-rubric.md` — Levels 4–5, extending `agent-readiness`'s 1–3.
...
- `agent-readiness` — scaffolds the project-context AGENTS.md that this skill's `promote`
  adds rules to. Pair them.
- `agent-experience` — the umbrella AX discipline; this is its evidence-and-feedback-loop arm.
