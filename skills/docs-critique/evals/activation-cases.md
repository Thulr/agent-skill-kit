# docs-critique Eval Cases

Activation + behavioral cases for `docs-critique` — critiquing *existing*
...
```bash
bash skills/docs-critique/evals/run-static-checks.sh
```
...
agent: activates for realistic docs-critique prompts; asks at most one blocker
...
**Expected:** recognizes end-user product flow (not help content); defers to `ux-critique`.
