...
- **Do not autogenerate from boilerplate or `/init` (W9).** Mündler et al. (arXiv:2602.11988) found
  ...
  also runs an evidence-driven feedback loop (`evidence-driven-agent-rules`), rules can
...
- **H5. Bootstrap discoverability when no AGENTS.md exists yet.** Before AGENTS.md lands,
  ...
  `evidence-driven-agent-rules`), the README §Agents block can include an optional pointer to
  `docs/reflection-log/` with the recording-bar / promotion-bar callout. The pointer is optional
  here; it's required by `evidence-driven-agent-rules` for projects that opt into that workflow.
...
> **W1** (failure-driven ≥3 floor) is owned by `evidence-driven-agent-rules`. Projects that run
...
The reflection-log templates moved to `evidence-driven-agent-rules` — use that skill if the
...
- "Evaluating AGENTS.md: Are Repository-Level Context Files Helpful for Coding Agents?" —
  ...
  no-autogenerate guard (W9). The same paper grounds W1 in `evidence-driven-agent-rules`.
