...
# ----- File presence (design/measure is lean: no severity/score/tracking) -----
check_file "$skill_md"
...
# Only templates that actually ship for this skill.
...
check_pattern 'design-doc has proposed structure' '^## Proposed structure' "$skill_dir/templates/design-doc.md"
check_pattern 'design-doc has acceptance criteria' '^## Acceptance criteria' "$skill_dir/templates/design-doc.md"
check_pattern 'measurement-plan has metrics' '^## Metrics' "$skill_dir/templates/measurement-plan.md"
check_pattern 'measurement-plan has gates and evals' '^## Gates and evals' "$skill_dir/templates/measurement-plan.md"

# ----- Intent router structure (design + measure only) -----
if [[ -f "$intent_router" ]]; then
  rows=$(grep -cE '^(design|measure),' "$intent_router")
...
for surface in $all_surfaces; do
  ...
  awk '/^## Heuristics/{f=1;next} /^## /{f=0} f && /\((audit|design|debug|measure)/{found=1} END{ if(!found) exit 1 }' \
    "$pb" || fail "$surface.md: Heuristics has no intent tags"
  wc=$(wc -w < "$pb")
  (( wc < 400 || wc > 1500 )) && fail "$surface.md word count $wc outside 400-1500"
done

# ----- Registry integrity (CSV -> playbook file) -----
# Playbook references are load-bearing and must resolve. core_refs are validated
# only for the files this design skill actually ships: docs-design is the lean
# (no-rubric) side, so the score/severity rubrics named by the intent CSVs are
# intentionally absent here (they are critique-local; see spec
# 2026-05-30-domain-function-split). Skip those two rather than fail.
skip_core_refs="references/core/score-rubric.md references/core/severity-rubric.md"
for intent in "${INTENTS[@]}"; do
  ...
  while IFS='|' read -r pb refs; do
    ...
    for r in "${rparts[@]}"; do
      [[ " $skip_core_refs " == *" $r "* ]] && continue
      [[ -f "$skill_dir/$r" ]] || fail "$intent.csv references missing core_ref: $r"
    done
