...
and resource context; semantic conventions supply standard attribute names (`code.filepath`,
`code.function`, `code.lineno`) so spans from different frameworks land in the same schema.
...
- **H4.** Verify OTel semantic conventions are used: `code.filepath`, `code.function`,
  `code.lineno` present; traces, logs, and metrics correlated via trace context + resource context.
...
- **H2.** Scaffold in three layers: (1) run metadata as OTel resource attributes (model, prompt
  ...
  `code.*` attributes; (3) replayable artifact export (`.traj` JSON or append-only event log).
