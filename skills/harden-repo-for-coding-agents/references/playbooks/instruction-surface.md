...
- **Do not autogenerate from boilerplate or `/init` (W9).** Mündler et al. (arXiv:2602.11988) found
  ...
  also runs an evidence-driven feedback loop (`rules-from-coding-agent-failures`), rules can
...
- **H5. Bootstrap discoverability when no AGENTS.md exists yet.** Before AGENTS.md lands,
  ...
  `rules-from-coding-agent-failures`), the README §Agents block can include an optional pointer to
...
> **W1** (failure-driven ≥3 floor) is owned by `rules-from-coding-agent-failures`. Projects that run
...
The reflection-log templates moved to `rules-from-coding-agent-failures` — use that skill if the
...
- "Evaluating AGENTS.md: Are Repository-Level Context Files Helpful for Coding Agents?" —
  ...
  no-autogenerate guard (W9). The same paper grounds W1 in `rules-from-coding-agent-failures`.
