...
>
> Apply the heuristics in `references/perf/playbooks/[surface].md` tagged for
> [intent]. Use the severity scale in
> `references/perf/core/severity-rubric.md`.
>
...
2. **Preserve disagreements** as open questions, not silent winners. If
   ...
   signal — surface it.
3. **Rank by severity** using `references/perf/core/severity-rubric.md`.
...
of `references/perf/intents/audit.csv` and fan out one sub-agent **per surface
...
- Loads `references/perf/playbooks/<surface>.md` and the core refs from its
  CSV row.
...
- Returns a per-surface finding list plus a 0–10 score from
  `references/perf/core/score-rubric.md` and a one-line "biggest gap" summary.
...
4. **Emit `templates/perf/audit-report-multi.md`** rather than the
