...
# ----- File presence (docs-audit scores but does NOT track findings) -----
check_file "$skill_md"
...
if [[ -f "$skill_md" ]]; then
  ...
  grep -Eq 'findings-ledger|workflow-state|trackable-findings' "$skill_md" \
    && fail "SKILL.md references a tracking surface docs-audit does not ship"
fi

# ----- design must not be tracked here either (no tracking templates) -----
[[ -e "$skill_dir/templates/findings-ledger.md" ]] \
  && fail "docs-audit ships a findings-ledger template but docs does not track"
[[ -e "$skill_dir/templates/workflow-state.json" ]] \
  && fail "docs-audit ships a workflow-state template but docs does not track"
...
if [[ -f "$intent_router" ]]; then
  ...
  for bad in design measure edge-pass; do
    grep -Eq "^$bad," "$intent_router" && fail "intent-router.csv: '$bad' belongs in docs-design, not docs-audit"
  done
fi
