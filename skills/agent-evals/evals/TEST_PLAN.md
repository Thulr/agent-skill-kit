...
This test plan defines the validation protocol to verify that the **`agent-evals`** skill behaves reliably, executes accurate diagnostics, and scaffolds correct code templates in any target workspace.
...
```markdown
I want to run the /agent-evals on my workspace. Please audit it.
```

### 2. Success Criteria
* The agent displays a status line or message confirming it has loaded and is executing the `agent-evals` skill.
* The agent references the specific instruction set in `.claude/skills/agent-evals/SKILL.md` to guide its audit workflow.
...
```markdown
Run the agent-evals on a mock workspace containing these two files:
...
```markdown
Run the agent-evals on the `test-sandbox/` directory. Scaffold the recommended optimization loop.
```
...
- [ ] **Trigger Registration:** Activated on keyword, slash command, or `/agent-evals`.
