...
# ----- Discover layer references (source of truth for surfaces) -----
all_surfaces=""
if [[ -d "$layer_dir" ]]; then
...
if [[ -f "$intent_router" ]]; then
  ...
  for bad in author strategize prune; do
    grep -Eq "^$bad," "$intent_router" && fail "intent-router.csv: '$bad' belongs in test-design, not test-critique"
  done
fi
