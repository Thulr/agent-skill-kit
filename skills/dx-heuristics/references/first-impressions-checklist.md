...
- Run through the list before scoring; record yes/no per item.
- Each "no" maps to a finding tagged with the relevant playbook below; use
  the playbook's severity rubric to set finding severity.
- A surface cannot score above 7/10 if more than two items are "no" — the
  basics gate the polish.
- Skip items that genuinely do not apply (e.g. no `--help` for a pure SDK).
  Document the skip; do not silently drop the item.
