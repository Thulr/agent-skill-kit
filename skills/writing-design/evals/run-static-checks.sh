...
for c in structure-rubric audience-frame draft-discipline clarity-rubric voice-guard \
         persuasion-rubric narrative-honesty-guard; do
...
check_file "$skill_dir/templates/outline-plan.md"
check_file "$skill_dir/templates/draft-scaffold.md"
check_file "$skill_dir/templates/persuasion-plan.md"
...
if [[ -f "$skill_md" ]]; then
  ...
  check_pattern 'routes critique to sibling' 'writing-audit' "$skill_md"
fi

# ----- template shapes -----
check_pattern 'outline-plan has outline section' '^## Outline' "$skill_dir/templates/outline-plan.md"
check_pattern 'draft-scaffold has rough first pass' '^## Rough first pass' "$skill_dir/templates/draft-scaffold.md"
check_pattern 'persuasion-plan has ABT spine' '^## ABT spine' "$skill_dir/templates/persuasion-plan.md"
check_pattern 'persuasion-plan has honesty check' '^## Honesty check' "$skill_dir/templates/persuasion-plan.md"

# ----- Intent router structure (design intents only) -----
if [[ -f "$intent_router" ]]; then
  rows=$(grep -cE '^(structure|draft|persuade),' "$intent_router")
  (( rows == 3 )) || fail "intent-router.csv: expected 3 design intent rows, got $rows"
  for i in "${INTENTS[@]}"; do check_pattern "$i intent" "^$i," "$intent_router"; done
  for bad in revise copyedit diagnose; do
    grep -Eq "^$bad," "$intent_router" && fail "intent-router.csv: '$bad' belongs in writing-audit, not writing-design"
  done
fi
...
for surface in $all_surfaces; do
  ...
  awk '/^## Heuristics/{f=1;next} /^## /{f=0} f && /\((structure|draft|persuade)/{found=1} END{ if(!found) exit 1 }' \
    "$pb" || fail "$surface.md: Heuristics has no design intent tags"
  wc=$(wc -w < "$pb")
