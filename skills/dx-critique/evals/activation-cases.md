# dx-critique Eval Cases

Activation + behavioral cases for `dx-critique` — critiquing an existing
...
```bash
bash skills/dx-critique/evals/run-static-checks.sh
```
...
**Expected:** routes (audit, all); stable `DX-<surface>-NNN` IDs; creates both `docs/audits/dx-critique-findings-ledger-…md` and `…workflow-state-…json` (or the `audit-artifacts/dx-critique-…` fallback); reports both paths.
**Fail if:** only offers to track, or emits the ledger inline without saving.

## Case 4 — Closeout from saved state
**Prompt:** `Verify whether DX-CLI-002 is fixed in this PR using docs/audits/dx-critique-workflow-state-2026-05-20-cli.json.`
...
**Expected:** recognizes operational/runtime perf, not developer-perceived perf; defers (perf-critique).
