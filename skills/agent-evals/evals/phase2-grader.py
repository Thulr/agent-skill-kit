"""Phase 2 grader for agent-evals.

Implements TEST_PLAN.md Phase 2: feeds mock workspace files (fixtures/) to
Claude under agent-evals's SKILL.md instructions, then uses a separate
...
Usage:
    python3 skills/agent-evals/evals/phase2-grader.py --dry-run
    python3 skills/agent-evals/evals/phase2-grader.py --live
    python3 skills/agent-evals/evals/phase2-grader.py --live --case agent-needs-sandbox
    python3 skills/agent-evals/evals/phase2-grader.py --live --model opus
...
VENV_DIR = REPO_ROOT / ".venv-agent-evals"  # shared with integration-test.sh
...
Below are the files in that workspace. Apply the agent-evals skill: scan,
...
def _ensure_anthropic() -> None:
    ...
    bootstrap `.venv-agent-evals/`, pip-install anthropic into it, and
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-bootstrap",
        action="store_true",
        help="Don't auto-create .venv-agent-evals/ if anthropic is missing. "
