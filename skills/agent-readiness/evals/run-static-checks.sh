#!/usr/bin/env bash
# Static checks for the agent-readiness skill.
...
echo "Checking agent-readiness skill at: $SKILL_DIR"

# 1. skill.json conforms to canonical schema and matches the directory.
validate_skill_json_contract "$REPO_ROOT" skill.json "agent-readiness"
...
if grep -q "agent-readiness-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md" SKILL.md &&
   grep -q "agent-readiness-workflow-state-<YYYY-MM-DD>-<scope-slug>.json" SKILL.md; then
  ok "SKILL.md uses skill-prefixed tracking filenames"
else
  err "SKILL.md missing agent-readiness-prefixed ledger/workflow-state paths"
fi
if grep -q "audit-artifacts/agent-readiness-" SKILL.md; then
  ok "SKILL.md preserves tracking fallback path"
else
  err "SKILL.md missing audit-artifacts/agent-readiness fallback path"
fi
if grep -q "^## Findings ledger" templates/assess-report.md &&
   grep -q "do not merely offer" templates/assess-report.md &&
   grep -q "audit-artifacts/agent-readiness-" templates/assess-report.md; then
...
validate_trigger_evals_contract "$REPO_ROOT" evals/trigger-evals.json "agent-readiness"
