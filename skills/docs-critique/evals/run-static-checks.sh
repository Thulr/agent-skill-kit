...
# ----- File presence (docs-critique scores but does NOT track findings) -----
check_file "$skill_md"
...
# Only templates that actually ship for this skill (no ledger/workflow-state).
...
if [[ -f "$skill_md" ]]; then
  ...
  check_pattern 'routes design elsewhere' 'docs-design' "$skill_md"
  # docs does NOT track findings: assert no tracking/ledger surface leaked in.
  grep -Eq 'findings-ledger|workflow-state|trackable-findings' "$skill_md" \
    && fail "SKILL.md references a tracking surface docs-critique does not ship"
fi

# ----- design must not be tracked here either (no tracking templates) -----
[[ -e "$skill_dir/templates/findings-ledger.md" ]] \
  && fail "docs-critique ships a findings-ledger template but docs does not track"
[[ -e "$skill_dir/templates/workflow-state.json" ]] \
  && fail "docs-critique ships a workflow-state template but docs does not track"

# ----- Intent router structure (critique = audit, debug) -----
if [[ -f "$intent_router" ]]; then
  rows=$(grep -cE '^(audit|debug),' "$intent_router")
  ...
  for bad in design measure edge-pass; do
    grep -Eq "^$bad," "$intent_router" && fail "intent-router.csv: '$bad' belongs in docs-design, not docs-critique"
  done
fi
...
# ----- Registry integrity (CSV -> playbook file; core_refs validated if present) -----
