...
- **Ann Cavoukian — *Privacy by Design: The 7 Foundational Principles*** —
  ...
  user action. User control and visibility: individuals must be able to see
  what is collected and revoke consent. Embed privacy into design: data
  minimisation and consent are architectural decisions, not bolt-ons.
...
- **Adam Wiggins — *The Twelve-Factor App*** — logs as event streams: the app
  emits structured events and the operator decides the sink and retention.
  Telemetry follows the same principle: the tool shapes and emits events; the
  destination and lifetime are the operator's concern, not hard-coded defaults.

## Good signals

- Telemetry is opt-in, or an unmissable first-run consent prompt explains what
  is collected before any data is sent.
- What is collected is documented in plain language with concrete field
  examples — not "anonymous usage data."
- Opt-out is a single command (`<tool> telemetry off` or equivalent), visibly
  confirmed; the inverse re-enables it.
...
- Opt-out works without a network call — it is a local operation only.
- Opt-out persists across upgrades; the tool does not silently re-enable
  telemetry on install.
- Response quality and latency are tracked over time, so a degraded-but-200
  upstream dependency surfaces as a trend alert rather than passing silently.

## Common failures

- Telemetry on by default with no notice at first run.
- "We collect anonymous usage data" with no concrete list of fields/events.
- Opt-out requires editing a config file the user doesn't know exists.
- PII such as emails, file paths, or environment variables leaks into payloads.
- Opt-out does not actually stop network calls — it only hides the UI
  indicator.
- Updates silently re-enable telemetry after the user has opted out.
- Data retention is unlimited or completely undocumented.
- A dependency quietly degrades (same 200, worse responses or higher latency)
  and nothing notices because the only watched signal is the status code, not
  quality or latency.
...
- **Detect silent provider degradation** *(audit, debug)* — a degraded-but-200
  response (correct shape, worse quality or higher latency) raises no error, so
  it is invisible to error handling. Detect it from telemetry: track
  quality/latency drift and alert on the trend, not the status code. The signal
  lives in the metrics stream, never an exception.
...
| Can a degraded-but-200 response be detected? | Only status code watched | Track quality/latency drift, alert on the trend |
