...
# ----- Intent router structure (critique = audit, debug, edge-pass) -----
if [[ -f "$intent_router" ]]; then
  rows=$(grep -cE '^(audit|debug|edge-pass),' "$intent_router")
  ...
  grep -Eq '^design,' "$intent_router" && fail "intent-router.csv: 'design' belongs in dx-design, not dx-critique"
fi
...
[[ -f "$cli_pb" ]] && check_pattern 'cli.md H1 title' '^# CLI Playbook' "$cli_pb"
