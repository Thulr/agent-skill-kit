...
Behavioral cases for the repo-local `informed-skill-reviewer` skill.
...
- "Review this draft and save a ledger for follow-up" -> assigns stable `SR-<area>-NNN` finding IDs and saves both `informed-skill-reviewer-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md` and `informed-skill-reviewer-workflow-state-<YYYY-MM-DD>-<scope-slug>.json`.
...
- Reviews with 7+ findings or any severity 3-4 finding save both tracking artifacts under `docs/audits/`, or use the matching `audit-artifacts/informed-skill-reviewer-...` fallback.
