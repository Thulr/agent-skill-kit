...
audits also emit `perf-critique-findings-ledger-<date>-<slug>.md` +
`perf-critique-workflow-state-<date>-<slug>.json`.
...
- **Bare invocation** (`"use perf-critique"`, `"perf audit"`, `"start"`): load
...
9. **Create, resume, or close tracking state.** For audit outputs with 7+
   ...
   artifacts now at
   `docs/audits/perf-critique-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md`
   and `docs/audits/perf-critique-workflow-state-<YYYY-MM-DD>-<scope-slug>.json`.
   Fall back to `audit-artifacts/perf-critique-{findings-ledger|workflow-state}-<YYYY-MM-DD>-<scope-slug>.{md|json}`
