...
> **If this project also runs an evidence-driven feedback loop** (reflection log via `rules-from-coding-agent-failures`), list any patterns already promoted into rules below. Otherwise this section is empty.
...
<!-- Reflection-log scaffolding is the job of `rules-from-coding-agent-failures`, not this skill. Only list reflection-log files here if the user is running both skills and EDAR's `capture` workflow is producing them. -->
