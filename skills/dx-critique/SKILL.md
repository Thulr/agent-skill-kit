---
name: dx-critique
description: Use to CRITIQUE an existing developer-experience surface — audit it for friction and score it, debug a reported developer-facing issue, or run a pre-ship edge-case risk pass. Covers APIs, SDKs, CLIs, developer docs, examples, errors, setup, local inner-loop, migrations, package contracts, contributor flows, auth, IDE integration, plugins, perf perception, telemetry, README/first-impressions, logging, config, and changelogs. Triggers on "DX review", "audit our API/SDK/CLI", "is this error message OK", "why do users hit this", "pre-1.0 risk pass". Do NOT use to DESIGN a new surface from scratch (use dx-design), for end-user product UX/accessibility (use ux-critique), or for AI-agent-facing surfaces, AGENTS.md, or AI/Agent SDK design (use design-for-agents).
license: MIT
---
...
a developer installs, calls, debugs, extends, or maintains. Provenance lives in
`skill.json`; this file is runtime routing only.

**Produces:** an intent-specific report — `audit-report.md` (or
`audit-report-multi.md` for surface = `all`) / `debug-runbook.md` /
`edge-checklist.md`; tracked audits also emit
`dx-critique-findings-ledger-<date>-<slug>.md` +
`dx-critique-workflow-state-<date>-<slug>.json`.
...
debug an avoidable setup issue, that is a DX problem worth a finding.

## Activation

- **Bare invocation** (`"use dx-critique"`, `"DX review"`, `"start"`): load
...
5. **Spawn sub-agents in parallel (default for `audit` and `edge-pass`).**
   Single-surface: one lens per agent — first-time integrator, maintainer,
   adversarial debugger. Audit + `all`: one surface per agent, each running the
   three lenses sequentially inside itself. See "Subagent dispatch"; fall back
   to sequential only if the host has no delegation primitive.
6. **Apply the playbook.** Use the heuristics tagged for this intent. For
   ...
   `debug`, rank hypotheses before naming fixes; for `edge-pass`, scan every
   category in the playbook. If sub-agents ran, synthesize their findings here.
...
9. **Create, resume, or close tracking state.** For audit/edge-pass outputs with
   ...
   and `docs/audits/dx-critique-workflow-state-<YYYY-MM-DD>-<scope-slug>.json`.
   Fall back to `audit-artifacts/dx-critique-{findings-ledger|workflow-state}-<YYYY-MM-DD>-<scope-slug>.{md|json}`
...
intent-specific load-bearing section (findings / prevention / re-run trigger),
