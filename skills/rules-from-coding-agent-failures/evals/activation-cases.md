# Activation cases — rules-from-coding-agent-failures
...
3. **"Score our repo against Level 4 / Level 5 of the Engineering Agents maturity rubric."** → `assess-l4l5`
   ...
   - For 7+ gaps, any level-ceiling blocker, or a save/track request, saves
     both `rules-from-coding-agent-failures-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md`
...
4. **"Set up an AGENTS.md for my repo."** — `harden-repo-for-coding-agents`'s job, not this skill's.
5. **"Make my repo work well with Claude Code."** — `harden-repo-for-coding-agents`'s job; this skill is the evidence-driven layer on top.
...
2. **"Generate AGENTS.md from our observed failures."** — borderline
   between this skill (uses the reflection log) and `harden-repo-for-coding-agents`
   (scaffolds AGENTS.md). The right answer: run
   `harden-repo-for-coding-agents` first to scaffold the project-context AGENTS.md,
