...
>
> Apply the heuristics in `references/test/layers/[layer].md` tagged for [activity]. Use the severity scale in `references/test/core/severity-rubric.md`. Tag each finding with one or more failure modes from `references/test/core/failure-modes.md`.
>
...
3. **Rank by severity** using `references/test/core/severity-rubric.md`.
4. **Tag failure modes** using `references/test/core/failure-modes.md`.
5. **Score 0–10** using `references/test/core/score-rubric.md` for review activity.
...
When the user picks `--surface=all` for a `review`, iterate the rows of `references/test/intents/review.csv` and fan out one sub-agent **per surface row** — do not hardcode the surface list here; the CSV is the source of truth and surfaces may be added or renamed. The orchestrator does **not** load the layer playbooks itself — each spawned surface agent loads only its own playbook (`references/test/layers/<surface>.md`).

### Each layer sub-agent

- Loads `references/test/layers/<layer>.md` and the core refs from its CSV row
...
- Returns a per-layer finding list + a 0–10 score from `references/test/core/score-rubric.md` and a one-line "biggest gap" summary
...
4. **Emit `templates/test/review-report-multi.md`** rather than the single-layer template; append each layer's full report at the end
