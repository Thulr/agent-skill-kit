...
- **Umbrella hand-off** (a variant of the one-layer shape used by a discipline
  ...
  *hand-off-to-a-sibling-skill* rows (empty `detail_files`; `notes` names the
  target top-level skill). Used by `design-for-agents`'s `ax-router.csv`, which
  both reviews AX surfaces and routes out to `codebase-agent-readiness` /
...
artifact (e.g. `research`'s `decide` intents). `codebase-agent-readiness`'s
