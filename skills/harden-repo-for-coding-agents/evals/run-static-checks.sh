#!/usr/bin/env bash
# Static checks for the harden-repo-for-coding-agents skill.
...
echo "Checking harden-repo-for-coding-agents skill at: $SKILL_DIR"

# 1. skill.json conforms to canonical schema and matches the directory.
validate_skill_json_contract "$REPO_ROOT" skill.json "harden-repo-for-coding-agents"
...
if grep -q "harden-repo-for-coding-agents-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md" SKILL.md &&
   grep -q "harden-repo-for-coding-agents-workflow-state-<YYYY-MM-DD>-<scope-slug>.json" SKILL.md; then
  ok "SKILL.md uses skill-prefixed tracking filenames"
else
  err "SKILL.md missing harden-repo-for-coding-agents-prefixed ledger/workflow-state paths"
fi
if grep -q "audit-artifacts/harden-repo-for-coding-agents-" SKILL.md; then
  ok "SKILL.md preserves tracking fallback path"
else
  err "SKILL.md missing audit-artifacts/harden-repo-for-coding-agents fallback path"
fi
if grep -q "^## Findings ledger" templates/assess-report.md &&
   grep -q "do not merely offer" templates/assess-report.md &&
   grep -q "audit-artifacts/harden-repo-for-coding-agents-" templates/assess-report.md; then
...
validate_trigger_evals_contract "$REPO_ROOT" evals/trigger-evals.json "harden-repo-for-coding-agents"
