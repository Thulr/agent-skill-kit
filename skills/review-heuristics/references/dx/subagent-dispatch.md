...
>
> Apply the heuristics in `references/dx/playbooks/[surface].md` tagged for
> [intent]. Use the severity scale in
> `references/dx/core/severity-rubric.md`.
>
...
3. **Rank by severity** using `references/dx/core/severity-rubric.md`.
...
rows of `references/dx/intents/audit.csv` and fan out one sub-agent **per
...
- Loads `references/dx/playbooks/<surface>.md` and the core refs from its
  CSV row.
...
- Returns a per-surface finding list plus a 0–10 score from
  `references/dx/core/score-rubric.md` and a one-line "biggest gap" summary.
...
4. **Emit `templates/dx/audit-report-multi.md`** rather than the
