...
1. "Make our docs better." → could be `docs-audit` / `docs-design` or `harden-repo-for-coding-agents` (docs-index surface for agents). Ask: "for human readers or for AI agents?"
...
- On a bare invocation (`"use harden-repo-for-coding-agents"`), the skill must:
...
- On `scaffold` invocation without project knowledge stated:
  ...
  - If the user requests reflection-log-driven scaffolding, redirect to
    `rules-from-coding-agent-failures` (this skill is project-context-first,
    not evidence-driven).
...
- On large `assess` output (7+ findings) or any severity 3–4 finding:
  ...
  - Save both `harden-repo-for-coding-agents-findings-ledger-<YYYY-MM-DD>-<scope-slug>.md`
    and `harden-repo-for-coding-agents-workflow-state-<YYYY-MM-DD>-<scope-slug>.json`
