...
if [[ -f "$skill_md" ]]; then
  ...
  check_pattern 'routes design elsewhere' 'perf-design' "$skill_md"
fi
...
# ----- Intent router structure (critique = audit, diagnose) -----
if [[ -f "$intent_router" ]]; then
  rows=$(grep -cE '^(audit|diagnose),' "$intent_router")
  ...
  for bad in design optimize strategize; do
    grep -Eq "^$bad," "$intent_router" && fail "intent-router.csv: '$bad' belongs in perf-design, not perf-critique"
  done
fi
