# architecture-audit Eval Cases

Activation + behavioral cases for `architecture-audit` — auditing an existing
...
```bash
bash skills/architecture-audit/evals/run-static-checks.sh
```
...
tracking gates (CA- IDs, `architecture-audit-` ledger prefix), and that the
...
`docs/audits/architecture-audit-findings-ledger-…md` and
`…workflow-state-…json` (or the `audit-artifacts/architecture-audit-…`
...
**Prompt:** `Verify whether CA-DEP-002 is fixed in this PR using docs/audits/architecture-audit-workflow-state-2026-05-22-billing.json.`
...
**Expected:** recognizes a DX surface, not internal code structure; defers to `dx-audit`.
