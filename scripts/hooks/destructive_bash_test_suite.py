#!/usr/bin/env python3
"""Shared test suite for block-destructive-bash.py adapters.
...
Wired into `just check` and CI (`.github/workflows/ci.yml`) through thin
per-harness adapter test files. Add a new fixture row here when a new bypass is
observed and logged under `docs/reflection-log/` (one file per bypass, by date).
"""
...
def _load_hook_module(hook_path: pathlib.Path):
...
CASES = [
    ...
    ("git -c remote.origin.push=+refs/heads/feature push origin", False,
     "config +refspec to unprotected branch is allowed"),
    ("git -c remote.origin.push=refs/heads/main push origin", False,
     "config refspec without + is not a force-push"),
    ("git -c user.name=x push origin feature", False,
     "unrelated -c config on a safe push"),
    ...
    ("git -c remote.origin.push=+refs/heads/main push origin", True,
     "-c remote.origin.push=+refs/heads/main config force-push"),
    ("git -c remote.origin.push=+main push origin", True,
     "-c remote.origin.push=+main config force-push"),
    ("git -cremote.origin.push=+refs/heads/master push origin", True,
     "packed -cremote.<name>.push config force-push"),
    ("rm -rf -- /", True, "rm -- /"),
...
def run_subprocess_smoke(hook_path: pathlib.Path, harness_subdir: str):
    ...
    with tempfile.TemporaryDirectory(prefix="hook-missing-policy-") as tmp:
        tmp_hook = pathlib.Path(tmp) / harness_subdir / "hooks" / hook_path.name
...
def run_suite(hook_path: pathlib.Path, harness_subdir: str) -> int:
    module = _load_hook_module(hook_path)

    unit_failures = run_unit_tests(module)
    smoke_failures = run_subprocess_smoke(hook_path, harness_subdir)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 2:
        print(
            "usage: destructive_bash_test_suite.py <hook-path> <harness-subdir>",
            file=sys.stderr,
        )
        return 2
    return run_suite(pathlib.Path(args[0]).resolve(), args[1])
