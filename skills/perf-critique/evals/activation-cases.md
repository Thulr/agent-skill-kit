# perf-critique Eval Cases

Activation + behavioral cases for `perf-critique` — critiquing the performance
...
facing or local build/inner-loop perf is `dx-critique`; end-user product UX is
`ux-critique`.

## Static verification

```bash
bash skills/perf-critique/evals/run-static-checks.sh
```
...
agent: activates for realistic perf-critique prompts; asks at most one blocker
...
**Expected:** routes (audit, all); fans out one agent per surface; stable `PERF-<surface>-NNN` IDs; scores each surface 0–10; creates both `docs/audits/perf-critique-findings-ledger-…md` and `…workflow-state-…json` (or the `audit-artifacts/perf-critique-…` fallback); reports both paths.
**Fail if:** only offers to track, or emits the ledger inline without saving.

## Case 4 — Closeout from saved state
**Prompt:** `Verify whether PERF-latency-002 is fixed in this PR using docs/audits/perf-critique-workflow-state-2026-05-20-checkout.json.`
...
**Expected:** recognizes developer inner-loop perf, not production runtime perf; defers to `dx-critique`.

## N4 — Developer-facing CLI ergonomics
**Prompt:** `Our CLI --help is dense and confusing — review it.`
**Expected:** recognizes DX, not production perf; defers to `dx-critique`.

## N5 — End-user product UX
**Prompt:** `Our consumer signup form has a 60% drop-off. Help me reduce friction.`
**Expected:** recognizes end-user audience; defers to `ux-critique`.
