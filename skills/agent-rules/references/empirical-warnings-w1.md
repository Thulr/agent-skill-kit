...
Sole-tenant of `agent-rules`. W1 belongs here (not in
the shared `_shared/empirical-warnings.md`) because it's the failure-driven
core of this skill and does not apply to `agent-readiness`'s
...
moot. Those repos should use `agent-readiness` for project-context-first
