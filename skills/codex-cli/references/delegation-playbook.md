...
questions, or prompt preparation with `codex exec`.
...
- sandbox and approval limits
- the desired output format
- any known uncertainty or suspected failure mode

Do not ask Codex to "look at everything" when a narrower question would work.
...
Codex for evidence about recurring agent mistakes and user workflow patterns.

Rules:

- Run from a neutral directory such as `$HOME` using
  `scripts/codex-cross-project-reflect.sh`; do not let the current repository
...
3. Do not invoke Codex.
4. Mention any data or secret risks the user should review first.

## Disagreement Handling

When Codex's answer conflicts with the calling agent's analysis:
