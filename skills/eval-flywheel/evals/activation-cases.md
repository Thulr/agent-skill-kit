...
**Expected:** skill defers to `codebase-agent-readiness` (the agent-readiness skill). eval-flywheel is for **AI integration points inside the workspace**, not for **the workspace's own ergonomics for coding agents**. If activated by mistake, it should explicitly disambiguate before acting.
