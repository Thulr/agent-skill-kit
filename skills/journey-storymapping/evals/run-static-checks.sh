...
SKILL_NAME="journey-storymapping"
# One-layer router: intent routes directly to a playbook (+ optional template).
INTENTS=(map-journey diagnose-experience narrate-concept)
...
check_file "$skill_dir/templates/story-map.md"
check_file "$skill_dir/templates/concept-story.md"
...
if [[ -f "$router" ]]; then
  ...
  rows=$(grep -cE '^(map-journey|diagnose-experience|narrate-concept),' "$router")
  ...
  [[ -d "$skill_dir/references/intents" ]] && fail "references/intents/ found: journey-storymapping is a one-layer router (no surface layer)"
