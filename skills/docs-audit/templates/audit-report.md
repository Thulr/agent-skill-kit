# Docs Experience Audit — <scope>

## Summary

- **Intent:** audit
- **Project tier:** <Prototype | Growing | Load-bearing> (per references/calibration.md)
- **Surface(s):** <foundations | dev-docs | readme | changelog | contributor | examples | ux-help | api-contracts | audience-conflicts | all>
- **Primary audience:** <persona>
- **Secondary / harmed audiences:** <personas or none>
- **Score:** <0-10, if requested>
- **Overall risk:** <low | medium | high | critical>

## Evidence inspected

- <doc/source/path/link inspected>
- <telemetry/user report/support ticket/eval, if any>
- **Evidence gaps:** <unknowns that limit confidence>

## Findings

| ID | Severity | Surface | Audience | Finding | Evidence | Recommendation | Verification |
|---|---:|---|---|---|---|---|---|
| DXDOC-001 | <0-4> | <surface> | <persona> | <mechanism> | <quote/path/signal> | <fix> | <test/check> |

## Audience conflicts

- <conflict if any: audience helped, audience harmed, proposed resolution>

## Prioritized fixes — Now

Sized to the **Project tier**: below Load-bearing, list only the proportional set
here; push deferred best-practice to "Later — as it grows".

1. <highest leverage fix and owner>
2. <next fix>
3. <measurement or gate>

## Later — as it grows

Best-practice that doesn't pay off at the current **Project tier**. Keep it as a
checklist to revisit when the project moves up a tier — don't file it as findings
now. At Load-bearing, write "none — full coverage applied".

- <deferred item> — worth doing at <Growing | Load-bearing>

## Verification plan

- <sample test / link check / accessibility check / retrieval eval / support query>

## Grounding

- <source names from skill.json.inspired_by relevant to applied playbooks>
