...
# One-layer router: intent routes directly to a playbook (+ optional template).
INTENTS=(frame-outcomes map-opportunities define-jobs test-assumptions scope-mvp)
...
check_file "$router"
for i in "${INTENTS[@]}"; do check_file "$playbook_dir/$i.md"; done
check_file "$skill_dir/templates/opportunity-solution-tree.md"
check_file "$skill_dir/templates/jtbd-job-map.md"
check_file "$skill_dir/templates/assumption-test-plan.md"
check_file "$skill_dir/templates/mvp-definition.md"
check_file "$skill_dir/evals/activation-cases.md"
check_file "$skill_dir/evals/trigger-evals.json"

# ----- Discover playbooks (file side; CSV holds the route vocabulary) -----
all_intents_on_disk=""
if [[ -d "$playbook_dir" ]]; then
  for pb in "$playbook_dir"/*.md; do
    [[ -e "$pb" ]] || continue
    all_intents_on_disk+="$(basename "$pb" .md) "
  done
fi
...
if [[ -f "$skill_json" ]]; then
  ...
  family=$(jq -r '.metadata.family' "$skill_json")
  [[ "$family" == "discovery" ]] || fail "skill.json metadata.family must be discovery, got $family"
fi
...
if [[ -f "$router" ]]; then
  ...
  rows=$(grep -cE '^(frame-outcomes|map-opportunities|define-jobs|test-assumptions|scope-mvp),' "$router")
  (( rows == 5 )) || fail "intent-router.csv: expected 5 data rows, got $rows"
  for i in "${INTENTS[@]}"; do check_pattern "$i intent" "^$i," "$router"; done
  # One-layer router: no per-intent surface CSVs should exist.
  [[ -d "$skill_dir/references/intents" ]] && fail "references/intents/ found: product-discovery is a one-layer router (no surface layer)"
...
[[ -z "$all_intents_on_disk" ]] && fail "no playbooks found in $playbook_dir"
for pb in "$playbook_dir"/*.md; do
  [[ -f "$pb" ]] || continue
...
for intent in $all_intents_on_disk; do
  pb_path="references/playbooks/${intent}.md"
  [[ -f "$router" ]] && grep -qF -- "$pb_path" "$router" \
    || fail "playbook $intent.md is not referenced by intent-router.csv (orphan)"
done
