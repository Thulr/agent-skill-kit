...
W1 is **owned by `agent-rules`**, not this skill. It is a
deliberate prose breadcrumb, not a symlink: W1 is that skill's sole-tenant file
and lives at
`skills/agent-rules/references/empirical-warnings-w1.md` — it is
...
route the *doing* to `agent-rules` (the evidence/feedback arm)
