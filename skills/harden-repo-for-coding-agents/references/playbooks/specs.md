...
- **H3.** Agent phases work horizontally despite spec → replace horizontal phases with
  ...
  `- [ ] <slice-name>: <user-visible outcome> (touches: <layer1>, <layer2>)`. Specify the slice's
  *outcome and success criteria*, not a step-by-step script — over-scripting the agent ("problems,
  not tasks") is its own failure mode. Ground each slice in the user's job and workflow, not just the
  ticket text.
...
- **github/spec-kit** — reference implementation of `constitution.md → spec.md → plan.md →
  tasks.md`; 30+ harness integrations; skills-mode install for Claude Code and Codex.
