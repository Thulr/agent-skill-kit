...
check_pattern 'frontmatter name' '^name:[[:space:]]*informed-skill-reviewer$' "$skill_md"
...
check_pattern 'ledger filename has skill prefix' 'informed-skill-reviewer-findings-ledger-<YYYY-MM-DD>-<scope-slug>\.md' "$skill_md"
check_pattern 'workflow-state filename has skill prefix' 'informed-skill-reviewer-workflow-state-<YYYY-MM-DD>-<scope-slug>\.json' "$skill_md"
check_pattern 'tracking fallback path is preserved' 'audit-artifacts/informed-skill-reviewer-' "$skill_md"
...
if (( failures > 0 )); then
  printf '\ninformed-skill-reviewer static eval failed with %d issue(s).\n' "$failures" >&2
  exit 1
fi

printf 'informed-skill-reviewer static eval passed.\n'
