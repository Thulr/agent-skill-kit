...
W1 is **owned by `evidence-driven-agent-rules`**, not this skill. It is a
deliberate prose breadcrumb, not a symlink: W1 is that skill's sole-tenant file
and lives at
`skills/evidence-driven-agent-rules/references/empirical-warnings-w1.md` — it is
...
route the *doing* to `evidence-driven-agent-rules` (the evidence/feedback arm)
