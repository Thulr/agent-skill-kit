...
(Analysis → Planning → Solutioning → Implementation) with 12+ agent personas (PM, Architect,
Developer, UX, Scrum Master, QA, Technical Writer), PRDs, ADRs (`docs/adr/NNNN-decision-name.md`),
...
- **Do not autogenerate specs, ADRs, or runbooks from templates without observed context.**
  LLM-generated context files drop task success ~3% and inflate cost >20% (W1). Every spec
  artifact must reference at least one stated requirement, failure mode, or architectural
  decision — no placeholder copy, no "TBD" sections shipped to production.
...
- **W1** — Autogenerated specs/ADRs drop task success ~3% and inflate cost >20%; hand-author from
  stated requirements or observed failures only.
...
- **BMAD-METHOD** — multi-persona orchestration with explicit handoff briefs between PM, Architect,
  Developer, QA, SM, UX, TechWriter; Party Mode for parallel persona sessions in one harness.
...
- "BMAD-METHOD" — 4-phase loop with 12+ agent personas; explicit handoff artifact pattern;
  quality gates between phases; multi-persona Party Mode.
