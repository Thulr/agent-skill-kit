# activation-cases.md — agent-evals

These cases check whether `agent-evals` (1) activates at the right time,
...
bash skills/agent-evals/evals/run-static-checks.sh               # just this skill

# ── Phase 2 — semantic diagnosis grader (free dry-run; ~$0.05 live) ───
python3 skills/agent-evals/evals/phase2-grader.py --dry-run      # preview prompts, no API
python3 skills/agent-evals/evals/phase2-grader.py --live         # grade cases against Claude
python3 skills/agent-evals/evals/phase2-grader.py --live --case agent-needs-sandbox
python3 skills/agent-evals/evals/phase2-grader.py --live --case traces-not-loop
python3 skills/agent-evals/evals/phase2-grader.py --live --case model-swap-benchmark
python3 skills/agent-evals/evals/phase2-grader.py --live --case post-readiness-operating-loop
python3 skills/agent-evals/evals/phase2-grader.py --live --model opus   # ~$0.25, Opus 4.7

# ── Phase 3 — sandbox scaffold + opt-in DSPy run (~$0.20 for execute) ─
bash skills/agent-evals/evals/integration-test.sh setup          # creates test-sandbox/src/classifier.py
...
bash skills/agent-evals/evals/integration-test.sh verify         # asserts ai-ops/ shape (free)
bash skills/agent-evals/evals/integration-test.sh execute        # real DSPy compile (~$0.20)
bash skills/agent-evals/evals/integration-test.sh teardown       # rm -rf test-sandbox/ + venv

# Re-running Phase 3 cleanly without paying for a dspy-ai reinstall:
bash skills/agent-evals/evals/integration-test.sh refresh        # teardown sandbox only, keep venv, re-setup
```
...
agent-evals in isolation. The hermetic version of Phase 3 is to
...
invocation auto-bootstraps `.venv-agent-evals/` and installs deps.
...
**Prompt:** `Run /agent-evals on this repo.`
...
**Prompt:** "We have Braintrust traces, LangSmith runs, thumbs-down feedback rows, and sampled production transcripts, but none of it changes prompts, evals, or release gates. Can agent-evals tell us what's missing?"
...
**Expected:** skill declines; routes the user to `ux-audit` if visible.
...
**Expected:** skill defers to `docs-audit`. Same disambiguation rule as E1.
