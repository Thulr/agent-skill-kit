...
**Surfaces audited:** <list from references/architecture/intents/audit.csv, excluding the `all` row — one entry per surface this audit actually visited>
**Date:** <YYYY-MM-DD>
...
One row per surface actually audited (drawn from `references/architecture/intents/audit.csv`, minus the `all` row).
