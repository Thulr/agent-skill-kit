...
SKILL_NAME="customer-interviewing"
# One-layer router: intent routes directly to a playbook (+ optional template).
INTENTS=(prep critique-questions conduct synthesize)
...
check_file "$skill_dir/templates/interview-plan.md"
check_file "$skill_dir/templates/interview-snapshot.md"
...
if [[ -f "$router" ]]; then
  ...
  rows=$(grep -cE '^(prep|critique-questions|conduct|synthesize),' "$router")
  ...
  [[ -d "$skill_dir/references/intents" ]] && fail "references/intents/ found: customer-interviewing is a one-layer router (no surface layer)"
