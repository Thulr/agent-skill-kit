...
python3 "$repo_root/scripts/check-skill-static.py" \
  ...
  --require-pattern 'audit report preserves fallback path::templates/audit-report.md::audit-artifacts/agent-test-' \
  --require-pattern 'report has Later/as-it-grows bucket::templates/audit-report.md::as it grows' \
  --require-pattern 'eval-design playbook H1::references/playbooks/eval-design.md::^# Eval Design Playbook'
